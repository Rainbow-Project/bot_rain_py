...
async def getZIP(mode: str, app: Ariadne, group: Group):
    random_name_ini = str(
        "".join(random.sample(string.ascii_letters + string.digits, 8)).strip()
    )
    ...
    async with PixivClient() as client:
        ...
        pyminizip.compress_multiple(
            files, zip_path, path_zip + f"{random_name}.zip", random_name, 9
        )
        await app.send_group_message(group, MessageChain("压缩包准备完成"))
        async with aiofiles.open(
            f"tmp/zip/{random_name}/{random_name}.zip", "rb"
        ) as aioFile:
...
@channel.use(
    ListenerSchema(
        ...
        inline_dispatchers=[
            Twilight([FullMatch("看看你的"), WildcardMatch(optional=True) @ "para"])
        ],
    )
)
