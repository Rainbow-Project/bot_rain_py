...
async def wows(app: Ariadne, group: Group, para: MatchResult):
    ...
    try:
        if lenCmd == 0:
            await app.sendMessage(group, MessageChain.create(str(random.randint(1, 100))))
        elif lenCmd == 1:
            await app.sendMessage(group, MessageChain.create(str(random.randint(1, int(cmd[0])))))
        elif lenCmd == 2:
            await app.sendMessage(group, MessageChain.create(str(random.randint(int(cmd[0]), int(cmd[1])))))
        elif lenCmd == 3:
            msg = ''
            ...
            await app.sendMessage(group, MessageChain.create('参数太多了！'))
    except Exception:
        await app.sendMessage(group, MessageChain.create('你确定输入没有问题？'))
