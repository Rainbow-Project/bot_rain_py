...
async def COVID(app: Ariadne, group: Group):
    ...
    await app.send_group_message(
        group,
        MessageChain(
            [Plain("新型冠状病毒前10:\n" + "\n".join(top_10)), Image(data_bytes=img)]
        ),
    )
...
async def get_COVID_19(pic=True):
    ...
    async with aiohttp.request(
        "GET", "https://c.m.163.com/ug/api/wuhan/app/data/list-total", headers=headers
    ) as r:
    ...
    y1 = [
        ...
        if a["today"]["confirm"] != None
        else a["total"]["confirm"]
    ...
    if pic:
        ...
        for a, b, c in zip(index, y1, y2):
            plt.text(a, b + c + 0.05, str(b + c), ha="center", va="bottom", fontsize=10)
